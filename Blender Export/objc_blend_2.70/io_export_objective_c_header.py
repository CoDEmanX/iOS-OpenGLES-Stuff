...
def triangulateNMesh(object):
    ...
    if bneedtri:
        ...
        # select all the face/vertex/edge
...
def do_export(context, props, filepath):
    mat_x90 = Matrix.Rotation(-math.pi / 2, 4, 'X')
...
typedef vertexDataTextured* vertexDataTexturedPtr;\n\n\n''')
        ...
        writeString(
            file,
            'static const vertexDataTextured %sVertexData[] = {\n' %
            basename)
...
typedef vertexDataColored* vertexDataColoredPtr;\n\n\n''')
        ...
        writeString(
            file,
            'static const vertexDataColored %sVertexData[] = {\n' %
            basename)
...
typedef vertexData* vertexDataPtr;\n\n\n''')
        ...
        writeString(
            file,
            'static const vertexData %sVertexData[] = {\n' %
            basename)
    ...
    if len(mesh.uv_textures) > 0:
        writeString(
            file,
            '// glTexCoordPointer(2, GL_FLOAT, sizeof(TexturedVertexData3D), '
            '&%sVertexData[0].texCoord);\n' %
            basename)
    elif len(mesh.vertex_colors) > 0:
        writeString(
            file,
            '// glColorPointer(4, GL_FLOAT, sizeof(ColoredVertexData3D), '
            '&%sVertexData[0].color);\n' %
            basename)

    writeString(
        file,
        '// glDrawArrays(GL_TRIANGLES, 0, k%sNumberOfVertices);\n' %
        basename)
